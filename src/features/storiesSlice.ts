...
export const fetchStories = createAsyncThunk("/stories/fetchStories", async () => {
    ...
    // const response = await fetch(`https://www.stuff.co.nz/static/spade/nCuL9ZmbMXzhGbHTJNJYU6i45y9hj0DJrhPteuU6MGB68zM5goWqk5Q1aNkh.json`);
    ...
    reducers: {
        ...
        sortByDateTime: (state) => {
            const sortedStories = state.filteredStories.sort(function (a, b) {
                const keyA = new Date(moment(a.publishedDate, "hh:mm DD MM YYYY").toDate()),
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchStories.fulfilled, (state, action) => {
                ...
                state.sections = Array.from(new Set(state.stories.map((item: IStory) => item.story.section)));
            })
